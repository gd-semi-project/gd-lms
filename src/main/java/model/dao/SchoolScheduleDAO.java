...
public class SchoolScheduleDAO {
    ...
    // =========================
    // 가장 가까운 일정 날짜 (start / end)
    // =========================
    public LocalDate findNearestScheduleDate(
            Connection conn,
            ScheduleCode code,
            LocalDate 기준일,
            boolean isStart
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            throw new RuntimeException(
                "가장 가까운 학사 일정 조회 실패: " + code, e
            );
        }

        throw new IllegalStateException(
            "다가오는 학사 일정이 존재하지 않습니다: " + code
        );
    }
    ...
    // 가장 가까운 일정 DTO
    // =========================
    public SchoolScheduleDTO findNearestSchedule(
            Connection conn,
            ScheduleCode code,
            LocalDate 기준일
    ) {
